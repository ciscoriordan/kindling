...
By default, dictionaries embed the kindlegen-derived ORDT/SPL collation tables in the orth index so Kindle folds diacritics at lookup time: looking up `meme` finds `même`, `mere` finds `mère`, etc. This matches kindlegen's behaviour and is what most users want. Pass `--strict-accents` to instead route the dictionary through the generated ORDT, whose minimal table leaves every letter as an out-of-table literal that the firmware matches exactly by code point, so accented and base forms no longer fold (e.g. `même` returns the `même` entry, not the `meme` entry). This is the same all-literal mechanism the generated-ORDT path below uses for Arabic/Chinese/Korean. The flag has no effect on book builds.
