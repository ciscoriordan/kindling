...
By default, dictionaries embed the kindlegen-derived ORDT/SPL collation tables in the orth index so Kindle folds diacritics at lookup time: looking up `meme` finds `même`, `mere` finds `mère`, etc. This matches kindlegen's behaviour and is what most users want. Pass `--strict-accents` to instead build a full per-character ORDT like kindlegen's: every character is its own symbol so the index keeps `ê` distinct from `e`, while accent and case variants share a single collation weight so accented headwords sort adjacent to their base form. The Kindle firmware collates Latin dictionaries with its own folding search, so the labels must be pre-sorted in that folded order; the distinct symbols then let it pick the exact form. The result is exact-accent matching on device: `même` returns the `même` entry and `mère` returns `mère`, instead of folding onto the unaccented form (issue #8). The flag has no effect on book builds.
