...
# series_book carries calibre:series / calibre:series_index in its OPF. kindlegen
# ignores both and emits no EXTH 112/113, which is what the parity test pins.
run_kg series_book series_book.opf

# The per-language dictionary fixtures (ja/zh/ko/ar/en/el/fr/ru/tr) live under
# tests/fixtures/langs/ and are regenerated by their own script, which builds
# both the kindling and kindlegen artifacts plus a test book per language:
#   KINDLEGEN=... KINDLING=target/release/kindling-cli \
#     python3 tests/fixtures/langs/generate.py
