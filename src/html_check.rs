...
///
/// This is a hand-rolled byte-level scan rather than a regex walk for a
/// few reasons:
///   1. Regex allocation on a 30MB blob is surprisingly slow.
///   2. We want precise byte offsets for diagnostics.
///   3. The corruption patterns we care about are very specific (past
///      regressions), so an explicit scan is easier to reason about.
pub fn check_no_corruption(content: &[u8]) -> Result<(), String> {
    ...
    // rather than using regex to keep allocation down on large blobs.
    let needle = b"<hr/";
    let mut i = 0;
...
///
/// Note: `assert_structural_tags_present` (tests-only) is NOT included
/// here. It asserts that `<html>`/`<body>` substrings exist, which is a
/// stronger requirement than what we want to enforce at build time (e.g.
/// very small test fixtures may not include them).
